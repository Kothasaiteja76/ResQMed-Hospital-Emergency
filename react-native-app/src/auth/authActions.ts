...
export async function loginWithPhone(phone: string, _otp: string): Promise<string> {
  ...
  if (!isFirebaseConfigured) {
    const stored = await getItem(DEMO_KEY);
...
export async function signupWithPhone(input: {
  ...
  if (!isFirebaseConfigured) {
    ...
    await setItem(DEMO_KEY, JSON.stringify({
...
export async function signup(input: { name: string; email: string; password: string }) {
  if (!isFirebaseConfigured) {
    ...
    await setItem(DEMO_KEY, JSON.stringify({ uid, email: input.email, displayName: input.name }));
    return;
  }
...
export async function login(input: { email: string; password: string }) {
  if (!isFirebaseConfigured) {
    const uid = `demo-${Date.now()}`;
    await setItem(DEMO_KEY, JSON.stringify({ uid, email: input.email, displayName: input.email.split('@')[0] }));
    return;
  }
...
export async function logout() {
  if (!isFirebaseConfigured) { await removeItem(DEMO_KEY); return; }
  await signOut(auth);
}
